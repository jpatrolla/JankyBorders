...
static bool parse_color(struct color_style *style, char *token) {
  if (sscanf(token, "=0x%x", &style->color) == 1) {
    ...
    return true;
  } else if (sscanf(token, "=gradient(top_left=0x%x,bottom_right=0x%x)",
                    &style->gradient.color1, &style->gradient.color2) == 2) {
    style->stype = COLOR_STYLE_GRADIENT;
    style->gradient.direction = TL_TO_BR;
    return true;
  } else if (sscanf(token, "=gradient(top_right=0x%x,bottom_left=0x%x)",
                    &style->gradient.color1, &style->gradient.color2) == 2) {
    style->stype = COLOR_STYLE_GRADIENT;
...
uint32_t parse_settings(struct settings *settings, int count,
                        char **arguments) {
  static char active_color[] = "active_color";
  static char floating_color[] = "floating_color";
  static char sticky_color[] = "sticky_color";
  ...
  for (int i = 0; i < count; i++) {
    if (str_starts_with(arguments[i], active_color)) {
      ...
      if (parse_color(&settings->inactive_window,
                      arguments[i] + strlen(floating_color))) {
        update_mask |= BORDER_UPDATE_MASK_FLOATING;
      }
    } else if (str_starts_with(arguments[i], sticky_color)) {
      if (parse_color(&settings->inactive_window,
                      arguments[i] + strlen(sticky_color))) {
        update_mask |= BORDER_UPDATE_MASK_STICKY;
      }
      ...
      settings->blacklist_enabled =
          parse_list(&settings->blacklist, arguments[i] + strlen(blacklist));
      update_mask |= BORDER_UPDATE_MASK_RECREATE_ALL;
    } else if (str_starts_with(arguments[i], whitelist)) {
      settings->whitelist_enabled =
          parse_list(&settings->whitelist, arguments[i] + strlen(whitelist));
      update_mask |= BORDER_UPDATE_MASK_RECREATE_ALL;
